...
void OctorokGolden_OnDeath(Entity* this) {
    CreateDeathFx(this, 0xff, 0);

    int flag = this->type2;
    int item = peek_item_for_global_flag_with_default(flag, ITEM_RUPEE100);

    Entity* entity = CreateObjectWithParent(this, GROUND_ITEM, get_item_id(item), get_item_subvalue(item));
    if (entity != NULL) {
        entity->timer = 5;
        entity->field_0x86.HWORD = global2flag(flag);
    }
    DeleteThisEntity();
}
